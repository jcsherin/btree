    ...
    public:
        ...
        bool DeleteOptimistic(const int keyToRemove) {
            ...
            while (current->GetType() != NodeType::LeafType) {
                ...
                auto iter = static_cast<InnerNode *>(node)->FindPivot(keyToRemove);
                current = iter->second;
            ...
            /**
             * This is the root(leaf) node. If it has only a single
             * key-value element then removing it will change the
             * root of this B+Tree. So we abort the optimistic
             * delete approach here and restart the process by
             * acquiring exclusive root latch and also exclusive
             * latch on the node itself.
             */
            if (parent == nullptr && node->GetCurrentSize() > 1) {
            ...
            /**
             * If removing this key-value element will cause this
             * leaf node to underflow then also we abort the optimistic
             * approach and restart the process by acquiring exclusive
             * latches from the root of the B+Tree down to this leaf
             * node.
             */
            ...
            /**
             * Optimistic approach failed. The process will restart by
             * acquiring exclusive latches from the B+Tree root.
             */
            root_latch_.LockExclusive();
            bool holds_root_latch = true;
            ...
            while (current->GetType() != NodeType::LeafType) {
                ...
                /**
                 * Release all parent locks if this node is safe.
                 *
                 * A node is safe when removing one key-value element
                 * from it will not cause it to underflow.
                 */
                if (node->GetCurrentSize() > static_cast<InnerNode *>(node)->GetMinSize()) {
                    while (!stack_traversed_nodes.empty()) {
                        (*stack_traversed_nodes.rbegin())->ReleaseNodeExclusiveLatch();
                        stack_traversed_nodes.pop_back();
                    }
                ...
                stack_traversed_nodes.push_back(current);
            ...
            node = reinterpret_cast<ElasticNode<KeyValuePair> *>(current);
            iter = static_cast<LeafNode *>(node)->FindLocation(keyToRemove);
            ...
            if (node == root_ && node->GetCurrentSize() == 0) {
                BPLUSTREE_ASSERT(holds_root_latch, "Hold exclusive root latch to be able to modify B+Tree root.");
            ...
            if (!stack_traversed_nodes.empty()) {
                auto parent = reinterpret_cast<ElasticNode<KeyNodePointerPair> *>(*stack_traversed_nodes.rbegin());
                stack_traversed_nodes.pop_back();
                ...
                if (parent->GetCurrentSize() >= static_cast<InnerNode *>(parent)->GetMinSize()) {
                    current->ReleaseNodeExclusiveLatch();
                    parent->ReleaseNodeExclusiveLatch();
            ...
            if (inner_node != nullptr) {
                ...
                BPLUSTREE_ASSERT(holds_root_latch, "Holds exclusive root latch for modifying B+Tree root");
